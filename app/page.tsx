...
const iconPaths: Record<IconName, string> = {
  ...
  network: "M6 8A2 2 0 1 0 6 4A2 2 0 0 0 6 8ZM18 8A2 2 0 1 0 18 4A2 2 0 0 0 18 8ZM12 20A2 2 0 1 0 12 16A2 2 0 0 0 12 20ZM8 7L10.5 16M16 7L13.5 16M8 6H16",
  ...
  truck: "M3 7H14V16H3V7ZM14 10H18L21 13V16H14V10ZM7 19A2 2 0 1 0 7 15A2 2 0 0 0 7 19ZM18 19A2 2 0 1 0 18 15A2 2 0 0 0 18 19Z",
};
...
function ProcurementDashboard() {
  ...
  const workflow = [
    ["Request intake", "Complete", 100],
    ["Supplier review", "In progress", 72],
    ["Commercial approval", "Pending", 48],
  ];

  return (
    <div className="mx-auto w-full max-w-2xl overflow-hidden rounded-[1.75rem] border border-[#171717]/10 bg-[#171717] p-3 shadow-2xl shadow-[#171717]/20 sm:p-4">
      <div className="rounded-[1.25rem] bg-[#fefaf1] p-4 sm:p-5">
        <div className="flex items-start justify-between gap-4 border-b border-[#171717]/10 pb-4">
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#171717]/45">
              Procurement Command Center
            </p>
            <h2 className="mt-1 text-2xl font-black">AARYX OS</h2>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {pipeline.map(([label, value, note]) => (
            <div
              ...
              key={label}
            >
              <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#171717]/45">
                {label}
              </p>
            ...
            <div className="mt-6 flex h-32 items-end gap-2">
              {[46, 64, 52, 78, 68, 92, 84].map((height, index) => (
                <span
                  ...
                  key={height + index}
                  style={{ height: `${height}%`, opacity: index % 2 ? 0.45 : 1 }}
                />
              ))}
            </div>
          </div>
        ...
        <div className="mt-3 rounded-2xl border border-[#171717]/10 bg-white p-4">
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            {["Procurement Requests", "Delivery Tracking", "Supplier Response"].map(
              (item, index) => (
                <div className="rounded-xl bg-[#f7f3ea] p-3" key={item}>
                  <p className="text-xs font-bold text-[#171717]/60">{item}</p>
...
export default function Page() {
  const trust = [
    ["Supplier Sourcing", "Structured sourcing support across trusted international supplier networks.", "network"],
    ["International Trade", "Operational support between Europe and high-growth Asian markets.", "globe"],
    ["Procurement Systems", "Digital infrastructure for RFQs, approvals and supplier management.", "package"],
    ["Operational Excellence", "Clear workflows built for quality, visibility and repeatable execution.", "shield"],
  ] as const;
      ...
      <header className="sticky top-0 z-50 border-b border-[#171717]/10 bg-[#f7f3ea]/90 backdrop-blur-xl">
        <div className="mx-auto flex min-h-14 w-full max-w-7xl items-center justify-between gap-4 px-5 py-2 sm:px-8">
          ...
          <nav className="hidden min-w-0 items-center gap-8 text-sm font-medium text-[#171717]/70 md:flex">
            {["Services", "Network", "Industries", "Founder", "Contact"].map((item) => (
              <a
