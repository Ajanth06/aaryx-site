...
export function HeroSection({
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-[#171717]/10">
      ...
      <div aria-hidden className="absolute -left-40 top-1/3 -z-10 size-[28rem] rounded-full bg-[#d24b2f]/10 blur-3xl" />
      ...
      <div className="mx-auto grid w-full max-w-7xl items-center gap-12 px-4 pb-16 pt-10 sm:px-5 sm:pb-24 sm:pt-14 lg:min-h-[calc(100vh-150px)] lg:grid-cols-[0.92fr_1.08fr] lg:gap-16 lg:px-8 lg:py-20">
        ...
        <div className="motion-safe:animate-[fade-up_700ms_ease-out_both]">
          ...
          <div className="inline-flex items-center gap-3 rounded-full border border-[#171717]/10 bg-white/70 px-4 py-2 shadow-sm backdrop-blur">
            <span className="size-2 rounded-full bg-[#d24b2f] shadow-[0_0_0_5px_rgba(210,75,47,0.12)]" />
          ...
          {/* Description */}
          <p className="mt-6 max-w-xl text-base font-medium leading-7 text-[#3f3a32]/72 sm:text-lg sm:leading-8 lg:text-xl lg:leading-9">
          ...
          <div className="mt-8 flex max-w-xl flex-col gap-3 sm:mt-10 sm:flex-row">
            ...
            <a
              className="inline-flex h-13 w-full items-center justify-center rounded-full border border-[#171717]/15 bg-white/60 px-7 text-sm font-bold text-[#171717] backdrop-blur transition hover:-translate-y-0.5 hover:border-[#171717]/35 hover:bg-white sm:w-auto"
              href={bookingHref}
              {...(bookingIsExternal
          ...
          {/* Fact chips — editorial left-border style */}
          <div className="mt-10 flex max-w-xl flex-col gap-3 min-[400px]:flex-row sm:mt-12">
            {dict.hero.facts.map((fact, i) => (
              <div
                className={`border-l-2 border-[#d24b2f] pl-4 py-0.5 motion-safe:animate-[fade-up_600ms_ease-out_both] stagger-${i + 1}`}
                key={fact.label}
              >
                <p className="text-sm font-black leading-tight sm:text-base">{fact.value}</p>
                <p className="mt-0.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#3f3a32]/50">
                  {fact.label}
                </p>
              </div>
            ))}
